...
### 1. Message Synchronization Claim
...
### 2. End Event Preservation Claim
...
这类 Claim 与已实现的 Action Preservation Claim 有重叠，但可以单独保留，因为 end event
对流程完整性和死锁分析都很关键。
...
### 7. Boundary Event Interruption Claim
...
1. Message Synchronization Claim：当前样例已有 messageFlow，策略支持也比较充分。
2. End Event Preservation Claim：实现简单，能增强流程结束语义检查。
3. Exclusive Branch Reachability Claim：与现有 mutex 互补。
4. Parallel Branch Preservation Claim：检查并行分支是否丢失。
5. Sequential Successor Claim：作为 causality 的局部补充，但要处理重复 Claim。
6. Parallel Join Synchronization Claim：需要更谨慎处理并行同步语义。
7. Boundary Event Interruption Claim：语义更复杂，适合后期扩展。
...
1. 实现 Message Synchronization Claims。
2. 实现 End Event Preservation Claims。
3. 实现 Exclusive Branch Reachability Claims。
4. 清理并明确中间产物生命周期。
5. 增加 mCRL2 action alphabet 解析。
6. 扩展 parallel gateway 相关 Claims。
7. 继续扩大 BPMN 特性覆盖范围。
