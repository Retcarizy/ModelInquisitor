...
1. 实现 Parallel Branch Preservation Claims。
2. 清理并明确中间产物生命周期。
3. 增加 mCRL2 action alphabet 解析。
4. 实现 Parallel Join Synchronization Claims。
5. 实现 Sequential Successor Claims。
6. 扩展 boundary event 等复杂 BPMN 语义。
7. 继续扩大 BPMN 特性覆盖范围。
