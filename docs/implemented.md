...
Claim 提取由 `ModelInquisitor/extractors/__init__.py` 统一组织。目前已经实现五类 Claim。
...
#### 3.3 Message Synchronization 检查

实现位于 `ModelInquisitor/extractors/message_synchronization.py`。

该提取器会为每条 BPMN `messageFlow` 生成一个 Claim，要求该消息在 mCRL2 模型中表现为同步后的 communicated action，而不是裸露的 send/receive action。
...
这类 Claim 用于发现通信同步规则缺失、`comm` 配置错误，或原始 send/receive 动作意外暴露等问题。
