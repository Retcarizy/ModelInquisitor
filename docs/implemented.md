...
#### 3.3 Message Flow Rendezvous 检查
...
对于 source/target 端直接连接到虚拟 `Environment` participant 的 message flow，提取器还会生成两类专门的 Environment Claims：

- `interaction::environment_rendezvous_visibility`：仍然在 LTS/PBES 层检查最终通信必须表现为 `c_msg`，并且裸露的 `s_msg` / `r_msg` 不应可见。
- `interaction::environment_endpoint_direction`：在生成的 mCRL2 源码层检查虚拟环境进程方向是否正确，即 `env_send_<flow_id>` 必须提供 `s_<message>(oid) . delta`，`env_recv_<flow_id>` 必须提供 `r_<message>(oid) . delta`。
